...
from ..data.data import Pedestrians, TimeIndexedPedData, RawData
...
class AutoEncoder(Module, Pedestrians):

    def __init__(self, config, encoder=None):
        super().__init__()
        self.config = config
        self.encoder = encoder # 
        self.diffnet = eval(config.diffnet) 
        ...
        self.diffusion = DiffusionTraj( 
            ...
            net = self.diffnet(config).to(config.device),
    ...
    def generate_multistep(self, data: TimeIndexedPedData, t_start=0, load_model=True):
        """
        Args:
            data:
        ...
        output = RawData(p_res, v_res, a_res, destination, destination, obstacles,
                                mask_p_new, meta_data=data.meta_data)
        return output, dest_force_res, ped_force_res
    
    def generate_multistep_geo(self, data: TimeIndexedPedData, t_start=0, load_model=True):
        """
        Args:
            data:
            t_start: rollout starts from frame #t
    ...
    def test_multiple_rollouts_for_training(self, data: TimeIndexedPedData, t_start=0):
        """
        dynamic weighting;
        Args:
            data:
    ...
    def test_multiple_rollouts_for_training_geo(self, data: TimeIndexedPedData, t_start=0):
        """
        Args:
            data:
            t_start:
