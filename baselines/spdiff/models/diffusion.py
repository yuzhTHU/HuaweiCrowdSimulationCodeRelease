...
class DiffusionTraj(Module):
    ...
    def denoise_fn(self, x_0, curr=None, context=None, timestep=0.08, t=None, mask=None):
        ...
        alpha_bar = self.var_sched.alpha_bars[t].to(x_0.device)
        ...
        e_rand = torch.randn_like(x_0).to(x_0.device)  # (B, N, d)******T
    ...
    def sample(self, context, curr = None, bestof=True, point_dim=2, timestep=0.08, ret_traj=False):
        ...
        for t in pbar:
            ...
            c0 = torch.sqrt(alpha_bar_).view(-1, 1, 1).to(curr.device)       # (B, 1, 1)
            c1 = torch.sqrt(1 - alpha_bar_).view(-1, 1, 1).to(curr.device)
        ...
        return torch.stack(traj_list).squeeze(0)
    ...
    def p_mean_variance(self, x_start, x_t, t):
        ...
        posterior_variance = self.var_sched.sigmas_inflex[t].expand(x_start.shape[0],*[1]*(x_start.ndim-1))
