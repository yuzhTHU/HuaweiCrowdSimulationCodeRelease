...
class RawData(object):
    ...
    def load_trajectory_data(self, data_path):
        ...
        self.position_desc = [position.masked_fill(mask_p.unsqueeze(-1).bool(),0).mean(),
                              position.masked_fill(mask_p.unsqueeze(-1).bool(),0).std()
                              ]
        self.velocity_desc = [velocity.masked_fill(mask_v.unsqueeze(-1).bool(),0).mean(), 
                              velocity.masked_fill(mask_v.unsqueeze(-1).bool(),0).std()
                              ]
        self.acceleration_desc = [acceleration.masked_fill(mask_a.unsqueeze(-1).bool(),0).mean(),
                              acceleration.masked_fill(mask_a.unsqueeze(-1).bool(),0).std()
                              ]
