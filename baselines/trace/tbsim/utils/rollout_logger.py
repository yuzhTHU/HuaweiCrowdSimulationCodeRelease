...
class RolloutLogger(object):
    ...
    def __init__(self, obs_keys=[], info_keys=["action_samples"]):
    ...
    def _combine_action(self, action: RolloutAction):
        ...
        if action.has_ego and not action.has_agents:
            ...
            if action.ego_info is not None:
                for kinfo in self._info_keys:
                    if kinfo in action.ego_info:
                        combined[kinfo] = action.ego_info[kinfo]
            return combined    
        elif action.has_agents and not action.has_ego:
            combined["action"] = action.agents.to_dict()
            if action.agents_info is not None:
                for kinfo in self._info_keys:
                    if kinfo in action.agents_info:
                        combined[kinfo] = action.agents_info[kinfo]
            return combined
            ...
            if action.agents_info is not None and action.ego_info is not None:
                for kinfo in self._info_keys:
                    if kinfo in action.ego_info:
                        ego_samples = action.ego_info[kinfo]
                    else:
                        ego_samples = None

                    if kinfo in action.agents_info:
                        agents_samples = action.agents_info[kinfo]
                    else:
                        agents_samples = None
                    ...
                    if ego_samples is not None and agents_samples is None:
                        ...
                        for k in ego_samples:
                            ...
                            combined[kinfo][k]=np.concatenate((ego_samples[k],pad_k),0)
                        ...
                        for k in agents_samples:
                            ...
                            combined[kinfo][k]=np.concatenate((pad_k,agents_samples[k]),0)
                        ...
                        for k in ego_samples:
                            if k in agents_samples:
                                ...
                                combined[kinfo][k] = np.concatenate((ego_samples[k],agents_samples[k]),0)
        return combined
    ...
    def _append_buffer(self, obs, action):
        ...
        state["action_traj_positions"] = action["action"]["positions"]
        state["action_traj_yaws"] = action["action"]["yaws"]
        if "action_samples" in action and "action_samples":

            state["action_sample_positions"] = action["action_samples"]["positions"]
            state["action_sample_yaws"] = action["action_samples"]["yaws"]
        if "diffusion_steps" in action:
            state["diffusion_steps_traj"] = action["diffusion_steps"]["traj"]
