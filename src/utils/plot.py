...
def plot_resilience(
    ...
    """
    Plot the resilience of a 2D function.
    - f: 2D function, such as lambda x, y: y - 3*y**2 - y**3 + x*y**3
...
class EqualizeNormalize(mcolors.Normalize):
    """Normalize by distribution rather than raw value."""
...
def clear_svg(path, debug=False):
    """
    SVG files generated by matplotlib may use syntax like
    <text style="font: 9.8px 'Arial'; text-anchor: middle" x="80.307802" y="193.900483">2020-02-02</text>,
    while PowerPoint cannot recognize the shorthand form
    `font: 9.8px 'Arial';` and only supports forms such as
    `font-family: 'Arial'; font-size: 9.8px;`.
    Therefore, conversion is required. The handled properties include:
    - font-size
    - font-family
    - font-weight
    - font-style
...
def load_font():
    ...
    plt.rcParams["font.sans-serif"] = [font_path]  # Specify the .otf file path here.
...
# Compute the nearest distance from a point to a box.
...
# Function for adjusting text positions.
def adjust_text(texts, ax, step=0.01, max_iterations=100, mode="xy"):
    ...
    for iteration in range(max_iterations):
        ...
        # Repulsion from i to j.
        F1 = cur_pos[j] - cur_pos[i]

        # Attraction from raw_pos[j] to j.
